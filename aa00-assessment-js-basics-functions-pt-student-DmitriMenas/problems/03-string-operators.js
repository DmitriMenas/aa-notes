...
function shortHandStr(str){
  let word = str 
  let first = word.toUpperCase().charAt(0);
  let last = word.toUpperCase().charAt(word.length - 1);
  return `${first}${last}`;
}
console.log(shortHandStr('apple'))
...
function makeFirstLowerCased(str1, str2){
  let change = str1.substring(str1.indexOf(str2), str1.indexOf(str2) + 1).toLowerCase()
  let start = str1.substring(0, str1.indexOf(str2))
  let end = str1.substring(str1.indexOf(str2) + 1)
  return `${start}${change}${end}`
}
console.log(makeFirstLowerCased('APPLE', "P"))
